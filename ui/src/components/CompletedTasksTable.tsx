...
function Row(props: RowProps) {
  ...
  const payloadDisplay = React.useMemo(
    () => formatPayload(task.payload),
    [task.payload]
  );
  const resultDisplay = React.useMemo(
    () => formatPayload(task.result ?? ""),
    [task.result]
  );
  ...
  return (
    ...
    >
      ...
      <TableCell>
        {renderPayloadCell(payloadDisplay)}
      </TableCell>
      <TableCell>{timeAgo(task.completed_at)}</TableCell>
      <TableCell>
        {renderPayloadCell(resultDisplay)}
      </TableCell>
      <TableCell>
        {task.ttl_seconds > 0
